...
typedef void (* kb_on_data_func_t)(kb_server * p_server,
                              kb_conn_t    p_conn,
                              const void * p_data,
                              uint32_t     p_len,
                              void *       p_user_data);

typedef void (* kb_on_connect_func_t)(kb_server* p_server,
                                 kb_conn_t p_conn,
                                 void *    p_user_data);

typedef void (* kb_on_disconnect_func_t)(kb_server * p_server,
                                    kb_conn_t p_conn,
                                    void *    p_user_data);
...
KB_API void kb_server_set_callbacks(kb_server *             p_server,
                                    kb_on_data_func_t       p_on_data,
                                    kb_on_connect_func_t    p_on_connect,
                                    kb_on_disconnect_func_t p_on_disconnect,
                                    void * p_user_data);

KB_API bool kb_server_start_async(kb_server * p_server, uint16_t p_port);
KB_API void kb_server_stop(kb_server * p_server);

KB_API bool kb_server_start_manual(kb_server * p_server, uint16_t p_port);
KB_API void kb_server_poll(kb_server * p_server);
